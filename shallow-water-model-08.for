...
C-----specify live dead regions (if needed)
      ...
      live(90:110,   1:80) = 0 ! dead
      live(90:110, 120:nj) = 0 ! dead
...
CC      xx = real(i-1)*dx
CC      h(i,j) = 0.02 - b(i,j) + 0.005*exp(-(xx-0.0)**2.0)

        if (i.le.90)  h(i,j) = 0.03
        if (i.ge.110) h(i,j) = 0.02
        if ((i.ge.90).and.(i.le.110)) h(i,j) = 0.025

        if ((i.ge.90).and.(i.le.110).and.
     &      (j.ge.81).and.(j.le.119)) then
          h(i,j) = 0.03 - 0.01*real(i-90)/20.0
        endif
...
C-----step5 Q-vector (full step)
     ...
     &             - dt*gg*hh*sb(n)
        ...
        if (u(i,j).gt.1.0) u(i,j) = 1.0
        if (v(i,j).gt.1.0) v(i,j) = 1.0
