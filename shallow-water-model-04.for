      ...
      integer iloc,jloc,i,j,n,ii,jj,it,sa

      real dx,dy,dt,rr,dtdx,dtdy,dzdx
...
CCC     rr = sqrt(dx*dx + dy*dy)
        rr = sqrt(dx*dx)

        if ((i.ge.90).and.(i.le.110)) then        
CCC       h(i,j) = 2.5 - b(i,j) + 0.1*(10.0 - rr) ! average height + peak
          h(i,j) = 40 - b(i,j)
        else
          h(i,j) = 30 - b(i,j) ! average height
        endif
