...
C-----specify live dead regions (if needed)
      ...
      live(45:55,  1:40) = 0 ! dead
      live(45:55, 60:nj) = 0 ! dead
...
CC      h(i,j) = 0.02 - b(i,j) + 0.005*exp(-(xx-0.0)**2.0)
        ...
        if (i.le.45) h(i,j) = 0.03
        if (i.ge.55) h(i,j) = 0.02

        if ((i.ge.45).and.(i.le.55)) then
          h(i,j) = 0.03 - 0.01*real(i-45)/10.0
        endif
